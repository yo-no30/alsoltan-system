<script setup lang="ts">
import AppBadge from '@/components/ui/AppBadge.vue'
import MoneyAmount from '@/components/ui/MoneyAmount.vue'
import type { Customer } from '@/types/database.types'

defineProps<{
  customers: Customer[]
  isBusy?: boolean
}>()

const emit = defineEmits<{
  edit: [customer: Customer]
  pay: [customer: Customer]
}>()

function formatDate(iso: string): string {
  try {
    return new Date(iso).toLocaleDateString('ar-SA')
  } catch {
    return iso
  }
}
</script>

<template>
  <div class="overflow-x-auto rounded-2xl border border-slate-200/70 bg-white shadow-sm">
    <table class="min-w-[44rem] text-sm">
      <thead class="border-b border-slate-200/70 bg-slate-50/80 text-slate-600">
        <tr>
          <th class="px-3 py-2 text-start font-medium">العميل</th>
          <th class="px-3 py-2 text-start font-medium">الجوال</th>
          <th class="px-3 py-2 text-start font-medium">ملاحظات</th>
          <th class="px-3 py-2 text-start font-medium">الذمة المستحقة</th>
          <th class="px-3 py-2 text-start font-medium">تاريخ الإضافة</th>
          <th class="px-3 py-2 text-start font-medium">إجراءات</th>
        </tr>
      </thead>
      <tbody>
        <tr v-if="customers.length === 0">
          <td colspan="6" class="px-4 py-10 text-center text-slate-400">
            لا يوجد عملاء بعد
          </td>
        </tr>
        <tr
          v-for="customer in customers"
          :key="customer.id"
          class="border-b border-slate-100 last:border-0"
        >
          <td class="px-3 py-2 font-medium text-slate-900">{{ customer.name }}</td>
          <td class="px-3 py-2 text-slate-600" dir="ltr">
            {{ customer.phone || '—' }}
          </td>
          <td class="max-w-[12rem] truncate px-3 py-2 text-slate-600">
            {{ customer.notes || '—' }}
          </td>
          <td class="px-3 py-2">
            <div class="flex items-center gap-2">
              <span
                class="font-semibold"
                :class="
                  Number(customer.balance_due) > 0
                    ? 'text-amber-700'
                    : 'text-slate-900'
                "
              >
                <MoneyAmount :amount="Number(customer.balance_due)" />
              </span>
              <AppBadge
                v-if="Number(customer.balance_due) > 0"
                variant="warning"
              >
                عليه ذمة
              </AppBadge>
            </div>
          </td>
          <td class="px-3 py-2 text-slate-600">
            {{ formatDate(customer.created_at) }}
          </td>
          <td class="px-3 py-2">
            <div class="flex flex-wrap gap-2">
              <button
                type="button"
                class="rounded-lg border border-slate-200/70 px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-60"
                :disabled="isBusy"
                @click="emit('edit', customer)"
              >
                تعديل
              </button>
              <button
                type="button"
                class="rounded-lg border border-brand-100 bg-brand-50 px-2.5 py-1.5 text-xs font-medium text-brand-700 hover:bg-brand-100 disabled:opacity-60"
                :disabled="isBusy || Number(customer.balance_due) <= 0"
                @click="emit('pay', customer)"
              >
                تسديد
              </button>
            </div>
          </td>
        </tr>
      </tbody>
    </table>
  </div>
</template>
