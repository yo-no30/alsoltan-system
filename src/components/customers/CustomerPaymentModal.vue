...
const balance = computed(() => Number(props.customer?.balance_due ?? 0))
...
function onSubmit(): void {
  ...
  if (parsed > balance.value) {
    error.value = 'المبلغ أكبر من الذمة المستحقة'
    return
  }
  error.value = ''
...
<template>
  <AppModal :open="open" title="تسديد ذمة عميل" @close="emit('close')">
    <div class="space-y-4">
      ...
      <p class="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
        الذمة الحالية: <MoneyAmount :amount="balance" />
      </p>
      <div>
